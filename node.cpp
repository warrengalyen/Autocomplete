...
vector<string> Node::getSuggestions(string str, string acc)
{
    ...
    for (auto &p : children) {
        vector<string> temp = children[p.first]->getSuggestions("", acc + p.first);
        for (auto &o : temp) {
            ans.emplace_back(o);
        }
    }
...
void Node::insertString(string str)
{
    if (str.length() == 0) {
        isTerminal = true;
        return;
    ...
    if (str.length() == 1) {
        children[str[0]]->insertString("");
    } else {
        children[str[0]]->insertString(str.substr(1));
    }
}
